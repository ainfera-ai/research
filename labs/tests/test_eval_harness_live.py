...
def test_cycle_refused_when_inert(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="INERT"):
        run_cycle(_taskset(), generated_at="2026-06-15T00:00:00Z")  # gateway=None → real path gated


def test_cycle_runs_with_fake_gateway(monkeypatch, tmp_path):
    monkeypatch.setenv(config.ARTIFACT_DIR_ENV, str(tmp_path))
    res = run_cycle(_taskset(), generated_at="2026-06-15T00:00:00Z", gateway=FakeGateway())
...
def test_web_snapshot_shape_matches_fixture():
    ...
    for tt in ("code", "summarize"):
        for arm, ok, c in [("A", True, 0.2), ("B", False, 0.01), ("C", True, 0.02), ("D", True, 0.08)]:
            calls.append(ArmCall(arm, f"{arm}-{tt}", tt, ok, c, config.LABS_TENANT, True, config.EVAL_RUN_TAG))
    web = snapshot.build_web_snapshot(
        version="v1", generated_at="2026-06-15T00:00:00Z",
        arm_metrics=metrics.compute_all(calls), type_table=metrics.by_arm_type(calls),
    )
    assert set(web) == {"version", "generated_at", "state", "source", "arms", "task_types"}
    assert set(web["arms"][0]) == {"id", "label", "kind", "cost_per_success", "success_rate"}
...
def test_assert_sanitized_rejects_leak():
    bad = {"state": "measured", "arms": [{"id": "C", "label": "Ainfera", "kind": "router",
            "cost_per_success": 0.02, "success_rate": 0.9, "model_used": "claude-opus-4-8"}]}
